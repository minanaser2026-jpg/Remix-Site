...
// Build Facebook embed URL from permalink
function getFbEmbedUrl(permalinkUrl: string): string {
  const encoded = encodeURIComponent(permalinkUrl);
  return `https://www.facebook.com/plugins/video.php?href=${encoded}&show_text=false&autoplay=true&width=500`;
}
...
function VideoModal({ reel, onClose }: { reel: Reel; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm bg-[#0d0d0d] rounded-2xl overflow-hidden border border-white/10 shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center rounded-full bg-black/60 text-white/70 hover:text-white hover:bg-[#CC0000] transition-colors"
        >
          <X className="w-4 h-4" />
        </button>

        {/* Facebook Embedded Player — 9:16 aspect ratio */}
        <div className="relative w-full" style={{ paddingBottom: '177.78%' }}>
          <iframe
            src={getFbEmbedUrl(reel.permalinkUrl)}
        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/50 text-xs font-sans">Facebook Reels</span>
          </div>
          <a
            href={reel.permalinkUrl}
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1 text-xs text-white/40 hover:text-white transition-colors font-sans"
          >
          ...
          <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {displayReels.map((reel, index) => (
              <button
                ...
                className="group relative aspect-[9/16] rounded-2xl overflow-hidden glass-card hover-glow block w-full text-left"
